...
(stdconstructor "Date" ((year :none) (month :none) (date :none)
  ...
  (mth "UTC" ((year :none) (month :none) (date :none)
              (hours :none) (minutes :none) (seconds :none) (ms :none))
...
(defun assert-date (val)
  (unless (dobj-p val) (js-error :type-error "Not a Date object.")))

(defmacro if-date ((val tvar &optional zonevar) &body then/else)
  (let ((v (gensym)))
    `(let ((,v ,val))
       (assert-date ,v)
...
(defun date-to-string (val flag &optional utc)
  (if-date (val time zone)
    (make-date-string time (if utc local-time:+utc-zone+ zone) flag)
    "Invalid Date"))

(defmacro date-setter (this unit val &key utc apply no-ret)
...
(defun make-date-string (timestamp zone type)
...
(let ((start-day (local-time:day-of (local-time:encode-timestamp 0 0 0 0 1 1 1970
                                                                 :timezone local-time:+utc-zone+))))
  (defun date-milliseconds (timestamp)
  ...
  (defun date-from-milliseconds (ms)
...
(stdproto (:date :object)
  (mth "toString" () (date-to-string this :full))
  (mth "toUTCString" () (date-to-string this :full t))
  (mth "toDateString" () (date-to-string this :date))
  (mth "toTimeString" () (date-to-string this :time))
  (mth "toLocaleString" () (date-to-string this :full))
  (mth "toLocaleDateString" () (date-to-string this :date))
  (mth "toLocaleTimeString" () (date-to-string this :time))
  (mth "valueOf" ()
  ...
  (mth "getUTCMilliseconds" ()
    (if-date (this time) (local-time:timestamp-millisecond time)))
  
  (mth "getTimezoneOffset" ()
  ...
  (mth "setFullYear" (year (month :none) (date :none))
  ...
  (mth "setUTCFullYear" (year (month :none) (date :none))
  ...
  (mth "setMonth" (month (date :none))
    (unless (eq date :none) (date-setter this :day-of-month date :no-ret t))
    (date-setter this :month month :apply (1+)))
  (mth "setUTCMonth" (month (date :none))
    (unless (eq date :none) (date-setter this :day-of-month date :utc t :no-ret t))
    (date-setter this :month month :apply (1+) :utc t))
  (mth "setDate" (date) (date-setter this :day-of-month date))
  (mth "setUTCDate" (date) (date-setter this :day-of-month date :utc t))
  (mth "setHours" (hour) (date-setter this :hours hour))
  (mth "setUTCHours" (hour) (date-setter this :hours hour :utc t))
  (mth "setMinutes" (min) (date-setter this :minute min))
  (mth "setUTCMinutes" (min) (date-setter this :minute min :utc t))
  (mth "setSeconds" (sec) (date-setter this :sec sec))
